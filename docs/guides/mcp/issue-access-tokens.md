...
type: guide
description: "Create, scope, rotate, and revoke the API keys that authenticate calls to MCP endpoints and the rest of the API."
prerequisites:
...
<AccordionGroup>
  <Accordion title="401 on every call with a newly created token">
    Check the prefix survived the copy. A token without the `aat_` prefix is
    routed to JWT verification, fails there, and reports the same 401 as a bad
    key. Shell quoting that drops the underscore is the usual cause.
  </Accordion>
  <Accordion title="401 after weeks of working">
    The key expired. `expires_at` is set at creation from `expires_in_days` and
    cannot be extended — `PATCH` is not supported on keys. Create a replacement.
  </Accordion>
  ...
  <Accordion title="A key you revoked still appears in the list">
    Revocation sets `is_active` to false rather than deleting the row, so the
    audit trail survives. Filter on `is_active` when you want live keys.
  </Accordion>
  <Accordion title="404 revoking a key by its prefix">
    `DELETE` takes the key's `id` , not its `token_prefix` . Look up the id from
    the list first.
  </Accordion>
  <Accordion title="You need per-endpoint scoping">
...
<Columns cols={2}>
  <Card title="Combine several MCP servers behind one endpoint" icon="plug" href="/guides/mcp/build-a-compound-mcp">
