...
class TestWorkspaceErrorHandling:
    ...
    async def test_cross_workspace_update_returns_none_not_error(self, repository, mock_session):
        ...
        # Arrange
        test_id = uuid4()
        repository.get_by_id = AsyncMock(return_value=None)  # Simulates cross-workspace access
    ...
    async def test_repository_create_database_error(self, repository, mock_session):
    ...
    async def test_error_message_does_not_leak_sensitive_info(self, jwt_handler, mock_request):
        ...
        assert "payload" not in exc_info.value.detail.lower()
