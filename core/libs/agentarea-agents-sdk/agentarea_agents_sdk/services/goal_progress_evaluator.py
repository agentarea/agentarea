...
class GoalProgressEvaluator:
    ...
    def _evaluate_progress_internal(
        ...
        try:
            ...
            if messages:
                ...
                goal_achieved, final_response, completion_method = self._check_explicit_completion(recent_messages)

                # Method 2: Check for single completion word (fallback for simple LLMs)
                if not goal_achieved:
                    goal_achieved, final_response, completion_method = self._check_single_word_completion(recent_messages)

                # Method 3: Fallback - check successful tool executions
                if not goal_achieved:
                    goal_achieved, final_response, completion_method = self._check_tool_pattern_completion(recent_messages)
            ...
            if message.get("role") == "tool" and message.get("name") == "completion":
                ...
                if isinstance(tool_content, dict):
                    if tool_content.get("completed", False):
                        return True, tool_content.get("result", "Task completed via tool"), "explicit_tool"
    ...
    def _check_single_word_completion(self, recent_messages: list[dict[str, Any]]) -> tuple[bool, str | None, str | None]:
    ...
    def _check_tool_pattern_completion(self, recent_messages: list[dict[str, Any]]) -> tuple[bool, str | None, str | None]:
            ...
            for message in reversed(recent_messages):
                if (
                    ...
                    if any(
                        ...
                        for word in ["summary", "result", "conclusion"]
                    ):
                        return True, content, "inference"
        return False, None, None

    def _evaluate_success_criteria(self, goal: dict[str, Any], messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Evaluate against success criteria if available."""
        success_criteria_met = []
