...
from agentarea.common.utils.types import (
    Task, TaskState, TaskStatus, Message, TextPart, 
...
class A2AChatService:
    ...
    async def send_chat_message(
        ...
        try:
            ...
            logger.info(f"Chat message sent successfully. Task ID: {task_id}, Context: {context_id}")
    ...
    async def get_conversation_history(self, context_id: str, limit: Optional[int] = None) -> List[Task]:
        """
        ...
        try:
            ...
            conversation_tasks = [
                ...
                if task.metadata and task.metadata.get("chat_message") and 
                (getattr(task, 'sessionId', None) == context_id or 
                 getattr(task, 'contextId', None) == context_id)
            ]
    ...
    async def get_user_conversations(self, user_id: str) -> List[str]:
        ...
        try:
            ...
            for task in user_tasks:
                if (task.metadata and task.metadata.get("chat_message") and 
                    task.metadata.get("user_id") == user_id):
    ...
    async def get_active_chat_tasks(self, agent_id: str) -> List[Task]:
        ...
        try:
            ...
            active_chat_tasks = [
                ...
                if (task.metadata and task.metadata.get("chat_message") and
                    task.status.state not in [TaskState.COMPLETED, TaskState.FAILED, TaskState.CANCELED])
            ]
