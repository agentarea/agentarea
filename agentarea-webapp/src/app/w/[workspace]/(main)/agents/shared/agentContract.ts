...
export function fromAgent(agent: AgentResponse): AgentFormValues {
  return {
    ...
    tools_config: toToolsConfig(agent.tools ?? []),
...
export function toAgentCreate(input: AgentFormValues): AgentCreateRequest {
  return { ...toAgentBody(input), triggers: input.triggers ?? [] };
}
