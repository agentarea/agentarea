...
export default function Table({data, columns}: {data: any[], columns: Column[]}) {
    return (
        <div className="relative flex flex-col h-full border rounded-lg card-shadow">
                ...
                <TableComponent>
                    <TableHeader>
                        <TableRow className="bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 border-zinc-200 dark:border-zinc-700">
                            {columns.map((column) => (
                                <TableHead key={column.accessor} className={cn("h-auto py-[3px] text-[11px] text-zinc-400 dark:text-zinc-400 uppercase font-medium first:pl-[20px] last:pr-[20px]", "last:rounded-tr-lg first:rounded-tl-lg", column.headerClassName)}>
                                    {column.header}
                                    {column.sortable && (
