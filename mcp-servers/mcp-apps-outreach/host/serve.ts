...
const HOST_PORT = Number.parseInt(process.env.HOST_PORT ?? "8490", 10);
const SANDBOX_PORT = Number.parseInt(process.env.SANDBOX_PORT ?? "8491", 10);
const MCP_URL = process.env.MCP_URL ?? "http://localhost:3402/mcp";
const ENTRY_TOOL = process.env.ENTRY_TOOL ?? "show_outreach_dashboard";
const DIST = path.join(import.meta.dirname, "..", "dist", "host");

const host = express();
host.get("/config.json", (_req, res) => {
  res.json({ mcpUrl: MCP_URL, sandboxUrl: `http://localhost:${SANDBOX_PORT}/sandbox.html`, entryTool: ENTRY_TOOL });
});
