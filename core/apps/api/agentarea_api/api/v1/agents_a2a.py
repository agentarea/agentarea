...
def log_a2a_operation(
    ...
    if auth_context.metadata:
        ...
        for key in ["user_agent", "client_ip", "forwarded_for"]:
            if key in auth_context.metadata and auth_context.metadata[key]:
...
def set_user_context_from_a2a_auth(auth_context: A2AAuthContext) -> None:
    ...
    logger.debug(f"Set user context for A2A request: user_id={user_id}, workspace_id={workspace_id}")
...
def create_error_response(request_id: str | int | None, error_code: int, error_message: str, error_data: Any = None) -> JSONRPCResponse:
...
def convert_a2a_message_to_task(
    ...
    a2a_metadata = {
        ...
        "monitoring": {
            ...
            "message_parts_count": len(message_params.message.parts) if message_params.message and message_params.message.parts else 0,
...
def convert_simple_task_to_a2a_task(task: SimpleTask) -> "Task":
    ...
    from agentarea_common.utils.types import Task, TaskStatus, TaskState
    ...
    try:
        ...
        log_a2a_operation(
            ...
            extra_metadata={
                ...
                "metadata_keys": list(created_task.metadata.keys()) if created_task.metadata else []
            }
        )
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("task_send", agent_id, auth_context, request_id, 
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("task_send", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=f"Invalid parameters: {e}")
        return create_error_response(request_id, -32602, f"Invalid parameters: {e}")
    except Exception as e:
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("task_send", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=f"Task submission failed: {e}")
        return create_error_response(request_id, -32603, f"Task submission failed: {e}")

async def handle_message_send(request_id, params, task_service, agent_id, auth_context, agent_service):
    ...
    try:
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("message_send", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=str(e))
        return create_error_response(request_id, e.code, e.message)
    except A2ATaskServiceError as e:
        duration_ms = (time.time() - start_time) * 1000
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("message_send", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=f"Message send failed: {e}")
        return create_error_response(request_id, -32603, f"Message send failed: {e}")

async def handle_message_stream_sse(request, request_id, params, task_service, agent_id, auth_context, agent_service):
    ...
    try:
        ...
        async def event_stream():
            ...
            try:
                ...
                async for event in task_service.stream_task_events(created_task.id, include_history=False):
                    event_count += 1
                    ...
                    if event_type in ["task_completed", "task_failed", "task_cancelled"] or \
                       event.get("event_type") in ["workflow.task_completed", "workflow.task_failed", "workflow.task_cancelled"]:
                        break
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("message_stream", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=str(e))
...
async def handle_task_get(request_id, params, task_service, agent_id, auth_context):
    ...
    try:
        ...
        if not task:
            duration_ms = (time.time() - start_time) * 1000
            log_a2a_operation("task_get", agent_id, auth_context, request_id, task_id,
                             status="failed", duration_ms=duration_ms, error="Task not found")
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("task_get", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=str(e))
        return create_error_response(request_id, e.code, e.message)
    except Exception as e:
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("task_get", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=f"Task get failed: {e}")
...
async def handle_task_cancel(request_id, params, task_service, agent_id, auth_context):
    ...
    try:
        ...
        # Log operation start
        log_a2a_operation("task_cancel", agent_id, auth_context, request_id, task_id, status="started")
        
        # Get task with current workflow status to ensure we have the most up-to-date status
        task = await task_service.get_task_with_workflow_status(task_id)
        if not task:
            duration_ms = (time.time() - start_time) * 1000
            log_a2a_operation("task_cancel", agent_id, auth_context, request_id, task_id,
                             status="failed", duration_ms=duration_ms, error="Task not found")
            return create_error_response(request_id, -32001, f"Task not found: {task_id}")
        ...
        if task.status in ["completed", "failed", "cancelled"]:
            ...
            log_a2a_operation("task_cancel", agent_id, auth_context, request_id, task_id,
                             status="failed", duration_ms=duration_ms, error=error_msg,
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("task_cancel", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=str(e))
        return create_error_response(request_id, e.code, e.message)
    except Exception as e:
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("task_cancel", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=f"Task cancel failed: {e}")
...
async def handle_agent_card(request_id, params, agent_service, agent_id, base_url, auth_context):
    ...
    try:
        ...
        enhanced_description = agent.description or f"AI agent powered by {agent.model_id or 'language model'}"
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("agent_card", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=str(e))
        return create_error_response(request_id, e.code, e.message)
    except Exception as e:
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("agent_card", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=duration_ms, error=f"Agent card retrieval failed: {e}")
...
async def _dispatch_rpc_method(
    ...
    *,
    ...
    handlers = {
        "tasks/send": lambda: handle_task_send(request_id, params, task_service, agent_id, auth_context, agent_service),
        "message/send": lambda: handle_message_send(request_id, params, task_service, agent_id, auth_context, agent_service),
        "message/stream": lambda: handle_message_stream_sse(request, request_id, params, task_service, agent_id, auth_context, agent_service),
        "tasks/get": lambda: handle_task_get(request_id, params, task_service, agent_id, auth_context),
        "tasks/cancel": lambda: handle_task_cancel(request_id, params, task_service, agent_id, auth_context),
        "agent/authenticatedExtendedCard": lambda: handle_agent_card(request_id, params, agent_service, agent_id, base_url, auth_context),
    }
    ...
    log_a2a_operation("unknown_method", agent_id, auth_context, request_id, 
                     status="failed", error=f"Method not found: {method}")
...
async def handle_agent_jsonrpc(
    ...
    try:
        ...
        if not body:
            log_a2a_operation("rpc_request", agent_id, auth_context, None, 
                             status="failed", error="Empty request body")
        ...
        try:
            ...
            log_a2a_operation("rpc_request", agent_id, auth_context, None, 
                             status="failed", error=f"JSON parse error: {e}")
        ...
        try:
            ...
            log_a2a_operation("rpc_request", agent_id, auth_context, request_data.get("id"), 
        ...
        log_a2a_operation(
            ...
            extra_metadata={
                ...
                "is_streaming_response": hasattr(result, 'media_type') and result.media_type == "text/event-stream"
            }
        )
        ...
        request_duration_ms = (time.time() - request_start_time) * 1000
        log_a2a_operation("rpc_request", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=request_duration_ms, 
                         error=str(e), extra_metadata={"method": method})
        return create_error_response(request_id, e.code, e.message)
    except A2ATaskServiceError as e:
        request_duration_ms = (time.time() - request_start_time) * 1000
        log_a2a_operation("rpc_request", agent_id, auth_context, request_id, 
        ...
        log_a2a_operation("rpc_request", agent_id, auth_context, request_id, 
                         status="failed", duration_ms=request_duration_ms, 
                         error=f"Internal error: {e}", extra_metadata={"method": method})
...
async def get_agent_well_known(
    ...
    log_a2a_operation("well_known", agent_id, auth_context, status="started",
                     extra_metadata={
                         "endpoint": "well-known",
                         "client_ip": request.client.host if request.client else None,
                         "user_agent": request.headers.get("user-agent")
    ...
    try:
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("well_known", agent_id, auth_context, 
                         status="failed", duration_ms=duration_ms, error=str(e))
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_a2a_operation("well_known", agent_id, auth_context, 
                         status="failed", duration_ms=duration_ms, error=f"Failed to get agent discovery info: {e}")
