...
def make_request(method: str, url: str, data: Optional[Dict[str, Any]] = None, timeout: int = 10) -> Dict[str, Any]:
...
def create_echo_mcp_instance(server_id: str) -> Optional[str]:
    ...
    response = make_request("POST", f"{CORE_API_BASE}/v1/mcp-server-instances/", instance_data)
    if response["success"]:
...
def wait_for_container_creation(instance_name: str, max_wait: int = 60) -> Optional[Dict[str, Any]]:
...
def test_echo_mcp_functionality(container_info: Dict[str, Any]) -> bool:
    ...
    container_port = container_info.get('environment', {}).get('MCP_CONTAINER_PORT', '3333')
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            if any(keyword in logs.lower() for keyword in ['server', 'listening', 'started', 'port']):
    ...
    try:
        ...
        result = subprocess.run(basic_test_cmd, shell=True, capture_output=True, text=True, timeout=10)
...
def test_environment_variables(container_info: Dict[str, Any]) -> bool:
    ...
    if missing_vars:
        ...
        if environment.get('PORT') == '3333' or environment.get('MCP_CONTAINER_PORT') == '3333':
            print("✅ Port configuration is correct (3333)")
        else:
            print(f"⚠️ Port configuration may be incorrect (PORT: {environment.get('PORT')}, MCP_CONTAINER_PORT: {environment.get('MCP_CONTAINER_PORT')})")
        
        # Check if MCP path is correctly set
        if environment.get('MCP_PATH') == '/mcp':
            print("✅ MCP path configuration is correct (/mcp)")
        else:
            print(f"⚠️ MCP path configuration may be incorrect (MCP_PATH: {environment.get('MCP_PATH')})")
...
def cleanup_test_data() -> None:
    ...
    for instance in instances:
        ...
        if name.startswith('test-') or name.startswith('event-test-') or name.startswith('echo-mcp-'):
            instance_id = instance.get('id')
            print(f"Deleting instance: {instance_id}")
            make_request("DELETE", f"{CORE_API_BASE}/v1/mcp-server-instances/{instance_id}")
...
def main() -> None:
    ...
    if container_info:
        ...
        print(f"   docker exec mcp-manager podman logs {container_info.get('name', 'CONTAINER_NAME')}")
        print(f"   curl -s http://localhost:7999/containers | jq '.containers[] | select(.service_name == \"echo-mcp-e2e-test\")'")
    else:
