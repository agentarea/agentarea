...
def test_wallet_lifecycle(alice_client: httpx.Client) -> None:
    ...
    created = alice_client.post(f"{alice_client.ws}/agents/{agent_id}/wallet", json=_wallet_payload())
    ...
    duplicate = alice_client.post(f"{alice_client.ws}/agents/{agent_id}/wallet", json=_wallet_payload())
    assert duplicate.status_code == 409

    fetched = alice_client.get(f"{alice_client.ws}/agents/{agent_id}/wallet")
    assert fetched.status_code == 200
    assert fetched.json()["id"] == wallet_id

    balance = alice_client.get(f"{alice_client.ws}/agents/{agent_id}/wallet/balance")
    assert balance.status_code == 200
    assert balance.json()["remaining"] == 12.5

    payments = alice_client.get(f"{alice_client.ws}/agents/{agent_id}/wallet/payments")
    assert payments.status_code == 200
    assert payments.json()["items"] == []

    funded = alice_client.post(
        f"{alice_client.ws}/agents/{agent_id}/wallet/fund",
    ...
    updated = alice_client.put(
        f"{alice_client.ws}/agents/{agent_id}/wallet",
    ...
    deleted = alice_client.delete(f"{alice_client.ws}/agents/{agent_id}/wallet")
    assert deleted.status_code == 204

    gone = alice_client.get(f"{alice_client.ws}/agents/{agent_id}/wallet")
...
def test_wallet_is_workspace_scoped(
    ...
    alice_client.post(
        f"{alice_client.ws}/agents/{alice_agent_id}/wallet", json=_wallet_payload()
    ).raise_for_status()

    bob_get = bob_client.get(f"{bob_client.ws}/agents/{alice_agent_id}/wallet")
    assert bob_get.status_code == 404

    bob_create = bob_client.post(
        f"{bob_client.ws}/agents/{alice_agent_id}/wallet", json=_wallet_payload()
    )
