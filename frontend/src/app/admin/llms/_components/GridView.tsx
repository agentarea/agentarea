      ...
      {list.map((item) => (
        <div key={item.id} className="card card-shadow group">
          <div className="flex flex-row items-start gap-[16px]">
            <div
              // className={cn(
              //   "h-[45px] min-w-[45px] w-[45px] rounded-md overflow-hidden group-hover:scale-105 transition-all duration-300 p-[3px]",
              //   item.image ? "dark:bg-white" : "bg-zinc-200 dark:bg-zinc-700"
              // )}
            >
              {/* {item.image && (
                <img
