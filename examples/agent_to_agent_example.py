...
async def create_task_for_agent(
    ...
    # Create payload
    payload = {"message": message, "agent_id": agent_id, "metadata": task_metadata}
...
async def wait_for_task_completion(
    task_id: str, max_wait_time: int = 60, poll_interval: int = 2
) -> Dict[str, Any]:
    """
...
async def run_primary_agent_with_helper(
    primary_agent_id: str, helper_agent_id: str, query: str
) -> Dict[str, Any]:
    """
...
async def simulate_agent_communication_locally(
    helper_agent_id: str, query: str
) -> None:
    """
    ...
    runner = Runner(app_name="agent_to_agent_example", agent=primary_agent)
    ...
    response = runner.run(user_id="example_user", new_message=content)
...
async def main():
    ...
    try:
        ...
        result1 = await run_primary_agent_with_helper(
            primary_agent_id, helper_agent_id, query1
        )
