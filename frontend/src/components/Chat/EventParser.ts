...
export const parseEventToMessage = (eventType: string, eventData: any): MessageComponentType | null => {
  const baseData = {
  ...
  switch (eventType) {
    ...
    case 'LLMCallFailed': {
      ...
      const providerType = originalData.provider_type || eventData.provider_type;
      ...
      } else if (originalData.is_rate_limit_error || eventData.is_rate_limit_error) {
      ...
      return {
        ...
        data: {
          ...
          raw_error: error,
          is_auth_error: originalData.is_auth_error || eventData.is_auth_error || false,
          is_rate_limit_error: originalData.is_rate_limit_error || eventData.is_rate_limit_error || false,
          is_quota_error: originalData.is_quota_error || eventData.is_quota_error || false,
          is_model_error: originalData.is_model_error || eventData.is_model_error || false,
          is_network_error: originalData.is_network_error || eventData.is_network_error || false,
          retryable: originalData.retryable !== false && eventData.retryable !== false  // Default to true
        }
      };
    }
    ...
    case 'WorkflowCompleted': {
      ...
      return {
        ...
        data: {
          ...
          success: true,
          iterations_completed: originalData.iterations_completed || eventData.iterations_completed,
    ...
    case 'WorkflowStarted': {
      const goalDescription = eventData.goal_description || eventData.original_data?.goal_description;
    ...
    case 'BudgetWarning': {
      const message = eventData.message || `Budget warning: ${eventData.usage_percentage}% used`;
      return {
        type: 'system',
        data: {
          ...baseData,
    ...
    case 'BudgetExceeded': {
      const message = eventData.message || `Budget exceeded: $${eventData.cost}/$${eventData.limit}`;
      return {
        type: 'system',
        data: {
          ...baseData,
