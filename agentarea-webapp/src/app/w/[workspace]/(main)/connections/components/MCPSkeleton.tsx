...
export function mcpSkeletonColumns(t: (key: string) => string): SkeletonColumn[] {
  return [
    { header: t("filters.type"), barClassName: "h-5 w-16 rounded-full" },
    { header: t("table.name"), barClassName: "h-4 w-40" },
    { header: t("table.description"), barClassName: "h-3 w-48" },
    { header: t("table.usedBy"), barClassName: "h-4 w-20" },
    { header: t("table.state"), barClassName: "h-5 w-20 rounded-full" },
  ];
}
