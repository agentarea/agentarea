...
test_agents = [
    ...
    },
    {
        ...
        "tools_config": {
            "mcp_server_configs": [{"mcp_server_id": "notion-server-uuid", "allowed_tools": []}],
            "builtin_tools": [
                {"tool_name": "calculator", "requires_user_confirmation": False, "enabled": True},
...
for agent in test_agents:
    ...
    print(
        f"Tools Config: {json.dumps(agent['tools_config'], indent=2) if agent['tools_config'] else 'None'}"
    )
    ...
    if tools_config:
        ...
        if tool_count > 0:
            print(
                f"Expected display: {builtin_count} builtin + {mcp_count} MCP = {tool_count} tool avatars"
            )
        else:
