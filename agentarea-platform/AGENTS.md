...
- Never let an installed extension override an EXPLICIT selector config (e.g. a `permissions` extension must not shadow `AGENTAREA_AUTHZ_BACKEND`); explicit config wins, extension is fallback. See EXTENSION POINTS above.
