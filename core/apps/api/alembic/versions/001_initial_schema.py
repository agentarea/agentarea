...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
        sa.Column('server_spec_id', sa.String(255), nullable=True),  # Changed from UUID FK to String
        sa.Column('name', sa.String(), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('trigger_id', postgresql.UUID(as_uuid=True), 
                 sa.ForeignKey('triggers.id', ondelete='CASCADE'), nullable=False),
    ...
    tables_with_workspace = [
        'agents', 'tasks', 'mcp_servers', 'mcp_server_instances',
        'provider_specs', 'provider_configs', 'model_specs', 'model_instances',
