...
def test_workspace_files_listing_includes_uploaded_project_file(
    ...
    listing = alice_client.get(f"{alice_client.ws}/files")
...
def test_workspace_files_are_isolated_per_workspace(
    ...
    bob_listing = bob_client.get(f"{bob_client.ws}/files")
...
def test_workspace_file_download_404_when_missing(
    ...
    resp = alice_client.get(f"{alice_client.ws}/files/never-existed.txt")
...
def test_workspace_file_path_traversal_is_refused(
    ...
    resp = alice_client.get(f"{alice_client.ws}/files/../../etc/passwd")
