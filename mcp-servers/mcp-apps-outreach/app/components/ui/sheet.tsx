...
/**
 * A side panel. In an MCP App the frame usually grows to fit the whole
 * dashboard, so "the viewport" can be thousands of pixels tall; callers pass
 * `style` with a `top`/`height` that keeps the panel where the user is looking.
 */
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        <SheetPrimitive.Close className="ring-offset-background focus:ring-ring data-[state=open]:bg-secondary absolute top-4 right-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none cursor-pointer">
