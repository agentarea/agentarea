...
class SourceService:
    ...
    async def upload_file(
        ...
        file_extension = (
            os.path.splitext(original_filename)[1] if original_filename else ""
        )
        ...
        metadata = {"size_bytes": len(file_content), "upload_timestamp": now}
    ...
    async def generate_presigned_url(
        self, filename: str, file_type: str, content_type: str = None
    ) -> dict:
        """
        ...
        if self.aws_settings.PUBLIC_S3_ENDPOINT and self.aws_settings.AWS_ENDPOINT_URL:
            ...
            presigned_url = presigned_url.replace(
                self.aws_settings.AWS_ENDPOINT_URL, self.aws_settings.PUBLIC_S3_ENDPOINT
            )
    ...
    async def create_source_after_upload(
        ...
        # Create metadata with file details
        metadata = {"size_bytes": file_size, "upload_timestamp": now}

        # Create the source record
        db_source = SourceModel(
            source_id=source_id,
            name=filename
            or f"Uploaded file {datetime.utcnow().strftime('%Y%m%d_%H%M%S')}",
            type=SourceType.FILE,
            description=description or f"Uploaded {file_type} file",
            configuration=configuration,
            meta_data=metadata,
