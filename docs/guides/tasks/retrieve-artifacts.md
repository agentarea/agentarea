...
type: guide
description: "List the files a task produced and download them through the API, with integrity verified before any bytes are returned."
prerequisites:
...
<Steps titleSize="h3">
  ...
  <Step title="Download everything">
...
<AccordionGroup>
  <Accordion title="The listing is empty on a completed task">
  ...
  <Accordion title="404 on anything outside `workspace/`">
    Only the `tasks/{task_id}/workspace/` subtree is downloadable through this
    endpoint. Other prefixes under the task return 404 by design.
  </Accordion>
  <Accordion title="The agent said it wrote a file but nothing is listed">
    ...
    is reported as an error on that tool result, so check the `tool.result`
    events for the call that created it — see
    [Debug a failed task](/guides/tasks/debug-a-failed-task) .
  </Accordion>
  <Accordion title="A large download is slow to start">
  ...
  <Accordion title="The `expires_in` query parameter appears to do nothing">
