...
# inventory.ini is generated by terraform (see "Provisioning the box"). For a
# host this repo does not own, copy inventory.example.ini instead.
...
with the code, and that now includes the VM: `terraform/` is the only root that
owns this machine. Two roots managing one server is the failure this avoids.

The infra repo (`agentarea-hq/infra`) owns the cluster and its VPC. The coupling
between them is one input, `vpc_network_id`, taken from `twc_vpc.cluster` in
`environments/production/timeweb-ru` — an id, not shared state, so neither root
can break the other.
