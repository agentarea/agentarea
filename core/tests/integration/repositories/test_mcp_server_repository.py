...
class TestMCPServerRepository:
    ...
    async def test_list_servers_by_status(self, db_session: AsyncSession):
        ...
        active_server = self.create_test_server(
            name="Active Server",
            status="active"
        )
        inactive_server = self.create_test_server(
            name="Inactive Server",
            status="inactive"
        )
        draft_server = self.create_test_server(
            name="Draft Server",
            status="draft"
        )
    ...
    async def test_list_servers_by_public_status(self, db_session: AsyncSession):
        ...
        public_server = self.create_test_server(
            name="Public Server",
            is_public=True
        )
        private_server = self.create_test_server(
            name="Private Server",
            is_public=False
        )
    ...
    async def test_list_servers_by_tag(self, db_session: AsyncSession):
        ...
        docker_server = self.create_test_server(
            name="Docker Server",
            tags=["docker", "container"]
        )
        python_server = self.create_test_server(
            name="Python Server",
            tags=["python", "scripting"]
        )
        mixed_server = self.create_test_server(
            name="Mixed Server",
            tags=["docker", "python", "utility"]
        )
    ...
    async def test_complex_filtering(self, db_session: AsyncSession):
        ...
        filtered_servers = await repository.list(
            status="active",
            is_public=True,
            tag="production"
        )
