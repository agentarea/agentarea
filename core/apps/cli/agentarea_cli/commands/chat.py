...
@click.pass_context
def send(ctx, agent_id: str, message: Optional[str], session_id: Optional[str], output_format: str):
...
@click.option("--format", "output_format", type=click.Choice(["text", "json"]), default="text", help="Output format")
@click.pass_context
def history(ctx, session_id: Optional[str], agent_id: Optional[str], limit: int, output_format: str):
    """Show chat history."""
...
@click.pass_context
def clear(ctx, session_id: Optional[str], force: bool):
    """Clear chat history."""
