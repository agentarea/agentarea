...
type: guide
description: "Diagnose a deployment that will not start, will not migrate, will not authenticate, or will not run tasks."
prerequisites:
...
<Steps titleSize="h3">
  <Step title="Establish which layer is broken">
  ...
  <Step title="Read the logs as structured data">
  ...
  <Step title="The database or migrations are wrong">
  ...
  <Step title="Tasks are created but never run">
  ...
  <Step title="File uploads or artifacts fail">
...
<AccordionGroup>
  <Accordion title="Everything is healthy and every action is denied">
  ...
  <Accordion title="`helm upgrade` succeeded and the platform runs the old version">
  ...
  <Accordion title="Logs are plain text instead of JSON">
    Something called `logging.basicConfig` after `setup_logging` , replacing the
...
<Columns cols={2}>
  ...
  <Card title="Collect logs and traces" icon="server" href="/self-host/observability">
