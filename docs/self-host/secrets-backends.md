...
<AccordionGroup>
  <Accordion title="API exits at startup with `SECRET_MANAGER_ENCRYPTION_KEY environment variable must be set`">
  ...
  <Accordion title="Agents fail on tool calls with a missing credential, but the UI shows the secret saved">
    The worker and the API disagree on `SECRET_MANAGER_TYPE` or on the key. Both
