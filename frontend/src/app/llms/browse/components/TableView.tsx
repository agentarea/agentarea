    ...
    const columns = [
        {
            header: "Name", accessor: "name", render: (value: string, item: LLMModel) => (
                <div className="font-semibold font-montserrat text-[14px] md:text-[16px] flex flex-col md:flex-row md:items-center gap-[10px] md:gap-[15px]">
                    <div className="w-[23px] h-[23px] md:w-[27px] md:h-[27px] p-[1px] rounded-sm overflow-hidden dark:bg-white">
        ...
        {header: "Category", accessor: "category", render: (value: string) => <div className="text-accent">{value}</div>},
        {
            header: "Description", 
            accessor: "description", 
            cellClassName: "text-[12px] md:text-[14px]", 
            render: (value: string) => <div className="line-clamp-3 md:line-clamp-none">{value}</div>
        },
    ];
