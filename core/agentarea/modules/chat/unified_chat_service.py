...
class UnifiedTaskService:
    ...
    async def list_sessions(
        ...
        for session_id, interactions in self.sessions.items():
            ...
            if user_id:
                user_interactions = [i for i in interactions if i.get("metadata", {}).get("user_id") == user_id]
            ...
            sessions.append({
                "id": session_id,
                "title": first_interaction["content"][:50] + "..." if len(first_interaction["content"]) > 50 else first_interaction["content"],
