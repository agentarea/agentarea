...
class TestAgentMCPE2E:
    ...
    def teardown_method(self):
        ...
        if self.task_id:
            requests.delete(f"{API_BASE_URL}/v1/workspaces/acme/tasks/{self.task_id}")
        if self.agent_id:
            requests.delete(f"{API_BASE_URL}/v1/workspaces/acme/agents/{self.agent_id}")
        if self.mcp_instance_id:
            requests.delete(f"{API_BASE_URL}/v1/workspaces/acme/mcp-server-instances/{self.mcp_instance_id}")
        if self.mcp_server_id:
            requests.delete(f"{API_BASE_URL}/v1/workspaces/acme/mcp-servers/{self.mcp_server_id}")
    ...
    def create_echo_mcp_server(self) -> dict[str, Any]:
        ...
        response = requests.post(f"{API_BASE_URL}/v1/workspaces/acme/mcp-servers/", json=server_data)
    ...
    def create_mcp_instance(self, server_id: str) -> dict[str, Any]:
        ...
        response = requests.post(f"{API_BASE_URL}/v1/workspaces/acme/mcp-server-instances/", json=instance_data)
        assert response.status_code in [200, 201]
    ...
    def execute_task_with_mcp(self, agent_id: str) -> dict[str, Any]:
        ...
        response = requests.post(f"{API_BASE_URL}/v1/workspaces/acme/tasks/", json=task_data)
        assert response.status_code in [200, 201]
        task = response.json()
    ...
    def _wait_for_instance_ready(self, instance_id: str, timeout: int = 60):
        ...
        response = requests.get(f"{API_BASE_URL}/v1/workspaces/acme/mcp-server-instances/{instance_id}")
    ...
    def _wait_for_task_completion(self, task_id: str, timeout: int = 120):
        ...
        while time.time() - start_time < timeout:
            response = requests.get(f"{API_BASE_URL}/v1/workspaces/acme/tasks/{task_id}")
            if response.status_code == 200:
                task = response.json()
    ...
    def test_agent_mcp_error_handling(self):
        ...
        if response.status_code == 201:
            ...
            task_response = requests.post(f"{API_BASE_URL}/v1/workspaces/acme/tasks/", json=task_data)
