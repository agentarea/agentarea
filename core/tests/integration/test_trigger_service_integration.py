...
class TestTriggerServiceIntegration:
    ...
    async def test_get_trigger_execution_repository_success(self, mock_db_session):
        ...
        assert (
            hasattr(repository, "get_by_trigger_id")
            or hasattr(repository, "list_by_trigger_id")
            or hasattr(repository, "list")
        )

    @pytest.mark.asyncio
    async def test_get_trigger_service_success(
        self, mock_db_session, mock_event_broker, mock_secret_manager
    ):
        ...
        with (
            patch(
                "agentarea_api.api.deps.services.get_trigger_repository"
            ) as mock_get_trigger_repo,
            patch(
                "agentarea_api.api.deps.services.get_trigger_execution_repository"
            ) as mock_get_exec_repo,
            ...
            patch(
                "agentarea_api.api.deps.services.get_model_instance_service"
            ...
            service = await get_trigger_service(
                mock_db_session, mock_event_broker, mock_secret_manager
            )
    ...
    async def test_get_webhook_manager_success(
        ...
        with patch("agentarea_api.api.deps.services.get_trigger_service") as mock_get_service:
            ...
            webhook_manager = await get_webhook_manager(
                mock_event_broker, mock_db_session, mock_secret_manager
            )
