...
down_revision: Union[str, None] = "add_llm_providers_fk"
...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE agents ALTER COLUMN planning TYPE BOOLEAN USING CASE WHEN planning = 'true' THEN true WHEN planning = 'false' THEN false ELSE null END"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE agents ALTER COLUMN planning TYPE VARCHAR USING CASE WHEN planning = true THEN 'true' WHEN planning = false THEN 'false' ELSE null END"
    )
