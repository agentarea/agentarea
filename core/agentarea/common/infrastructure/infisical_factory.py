...
def create_infisical_secret_manager() -> BaseSecretManager:
    ...
    try:
        ...
        logger.info(
            f"Initialized Infisical secret manager for project {project_id} in {environment}"
        )
        ...
        logger.error(
            f"Failed to initialize Infisical secret manager: {e}. Using local file manager."
        )
        return LocalSecretManager()
