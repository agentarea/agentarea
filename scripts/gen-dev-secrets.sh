...
managed_keys='AGENTAREA_AUTH_JWKS_B64
...
needs_write() {
  ...
  # AGENTAREA_AUTH_JWKS_B64 holds only the public half; Kratos signs with the private
  ...
  if [ "$1" = AGENTAREA_AUTH_JWKS_B64 ] && [ ! -f "$jwks_file" ]; then return 0; fi
  return 1
}
...
value_for() {
  case "$1" in
    ...
    AGENTAREA_AUTH_JWKS_B64) generate_jwks "$jwks_file" ;;
    # HMAC keys, not Ory cipher secrets: no 32-character constraint.
    AGENTAREA_SBX_ACTIVATION_SECRET|AGENTAREA_SBX_CLEANUP_SECRET) random_token 32 ;;
    *) random_secret_32 ;;
  esac
}

{
  ...
  for key in $pending; do
    if [ "$key" = AGENTAREA_AUTH_JWKS_B64 ]; then
...
if printf '%s' "$pending" | grep -q '^AGENTAREA_AUTH_JWKS_B64$'; then
