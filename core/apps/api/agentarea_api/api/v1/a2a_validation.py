...
class A2AValidator:
    ...
    def validate_json_rpc_request(cls, data: Dict[str, Any]) -> JSONRPCRequest:
        ...
        try:
            ...
            raise A2AValidationError(
                f"Invalid JSON-RPC request format: {e}",
                "INVALID_REQUEST"
            )
    ...
    def validate_agent_id(cls, agent_id: str) -> UUID:
        ...
        try:
            ...
            raise A2AValidationError(
                f"Invalid agent ID format: {agent_id}. Expected UUID format.",
                "INVALID_AGENT_ID"
            )
...
async def validate_a2a_middleware(request: Request, agent_id: UUID) -> Optional[Dict[str, Any]]:
