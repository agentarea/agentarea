...
class WorkflowTaskExecutionService:
    ...
    def _create_temporal_executor(self) -> TaskExecutorInterface:
        ...
        logger.info(f"Creating Temporal executor with server: {workflow_settings.TEMPORAL_SERVER_URL}, namespace: {workflow_settings.TEMPORAL_NAMESPACE}")
    ...
    async def execute_task_async(
        ...
        try:
            ...
            logger.error("To start Temporal server locally: docker run --rm -p 7233:7233 temporalio/temporal-auto-setup")
            raise
        except Exception as e:
