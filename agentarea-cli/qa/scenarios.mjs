...
function evidence(res) {
	if (isErr(res))
		return `status=${res.json.status} ${JSON.stringify(res.json.error).slice(
			0,
			200,
		)}`;
...
function sHealth() {
	const r = spawnSync('curl', ['-s', 'http://localhost:8000/health'], {
		encoding: 'utf8',
	});
...
function s2Agents() {
	...
	if (!id) {
		...
		const upd = cli([
			...
			id,
			'--data',
			JSON.stringify({description: 'qa-updated'}),
		]);
		check(
			'S2',
			'agent-update',
			isOk(upd) && upd.json.description === 'qa-updated',
			evidence(upd),
		);
		...
		check(
			'S2',
			'agent-list-contains',
			list.out.includes(id),
			`list length=${asArray(list.json).length}`,
		);
		...
		if (delOk)
			teardown.splice(
				teardown.findIndex(t => t.args[2] === id),
				1,
			);
	}
	...
	check(
		'S2',
		'neg-create-missing-name-validation',
		isErr(noName) && st >= 400 && st < 500,
		`expected 4xx validation, got ${evidence(noName)}`,
	);
	...
	check(
		'S2',
		'neg-foreign-agent-denied',
		isErr(foreign) && (fst === 403 || fst === 404),
		`expected 403/404, got ${evidence(foreign)}`,
	);
}
...
function s3Task() {
	const submit = cli(
		['tasks', 'submit', AGENT_REAL, 'Say hello in exactly one word'],
		{timeout: 120000},
	);
	const reachedComplete = submit.out.includes('WorkflowCompleted');
	check(
		'S3',
		'task-sse-workflow-completed',
		reachedComplete,
		reachedComplete
			? 'SSE stream contained WorkflowCompleted'
			: submit.out.slice(-200) || submit.err.slice(-200),
	);
	...
	if (!taskId) {
		skip(
			'S3',
			'task-db-status-completed',
			'could not extract task_id from stream',
		);
	...
	check(
		...
		'task-db-status-completed',
		dbStatus === 'completed',
		`tasks.status=${dbStatus || '(none)'} id=${taskId}`,
	);

	const evCount =
		parseInt(
			db(`SELECT count(*) FROM task_events WHERE task_id='${taskId}';`),
			10,
		) || 0;
	check(
		'S3',
	...
	const getStatus = isOk(get)
		? get.json.status ?? get.json.state
		: evidence(get);
	const statusStatus = isOk(status)
		? status.json.status ?? status.json.state
		: evidence(status);
	const consistent =
		String(getStatus) === String(dbStatus) &&
		String(statusStatus) === String(dbStatus);
	check(
		'S3',
		'task-status-vs-get-vs-db',
		consistent,
		`db=${dbStatus} get=${getStatus} status=${statusStatus}`,
	);
	if (!consistent) {
		finding(
			'Task status discrepancy across surfaces (S3)',
			`DB tasks.status='${dbStatus}', 'tasks get'.status='${getStatus}', 'tasks status'.status='${statusStatus}' for a task whose SSE reported WorkflowCompleted. task_id=${taskId}`,
		);
	}
}
...
function s4Mcp() {
	...
	check(
		'S4',
		'mcp-instances-list',
		isOk(list) && asArray(list.json).length >= 0,
		`count=${asArray(list.json).length}`,
	);

	const disc = cli(['mcp-instances', 'discover-tools', MCP_INSTANCE], {
		timeout: 60000,
	});
	...
	check(
		'S4',
		'mcp-discover-tools',
		isOk(disc) && tools.length > 0,
		isOk(disc)
			? `tools=${tools.length} first=${tools[0]?.name}`
			: evidence(disc),
	);

	skip(
		'S4',
		'mcp-create-instance-from-spec',
		'requires a server spec + running MCP infra to provision safely; used existing running instance for discover-tools instead',
	);
}
...
function s5Policy() {
	...
	const prev = cli([
		...
		'preview-policy',
		'--data',
		JSON.stringify({agent_id: AGENT_REAL}),
	]);
	if (isOk(prev)) {
		const blob = JSON.stringify(prev.json);
		const reflectsDeny =
			blob.includes(`${PREFIX}denied_tool`) || /deny/i.test(blob);
		check(
			'S5',
			'policy-enforcement-observable',
			reflectsDeny,
			`preview reflects policy: ${blob.slice(0, 200)}`,
		);
		if (!reflectsDeny)
			finding(
				'Policy not reflected in effective-policy preview (S5)',
				`Created deny policy on ${
					body.target
				} for agent ${AGENT_REAL}, but preview did not surface it: ${blob.slice(
					0,
					300,
				)}`,
			);
		...
		check(
			...
			'policy-enforcement-observable',
			false,
			`governance preview not usable: ${evidence(prev)}`,
		);
		finding(
			'Effective-policy preview not observable via API (S5)',
			evidence(prev),
		);
	}
	...
	check(
		'S5',
		'neg-policy-missing-fields-validation',
		isErr(bad) && st >= 400 && st < 500,
		`expected 4xx, got ${evidence(bad)}`,
	);
...
function s6Triggers() {
	...
	if (id) {
		...
		check(
			'S6',
			'trigger-executions-list',
			execs.code === 0 && !isErr(execs),
			evidence(execs),
		);
	...
	if (isOk(bad)) {
		...
		if (bad.json.id)
			trackDelete('triggers.delete', ['triggers', 'delete', bad.json.id]);
		check(
			'S6',
			'neg-malformed-cron-validation',
			false,
			`bad cron accepted: id=${bad.json.id}`,
		);
		finding(
			'Malformed cron accepted without validation (S6)',
			`cron_expression='not-a-cron-expr' created trigger ${bad.json.id}`,
		);
	} else {
		const st = errStatus(bad);
		check(
...
function s7Access() {
	...
	check(
		'S7',
		'apikey-create',
		isOk(create) && !!tokenId && !!create.json.token,
		isOk(create)
			? `id=${tokenId} token_prefix=${create.json.token_prefix}`
			: evidence(create),
	);
	...
	if (tokenId) {
		...
		check(
			'S7',
			'apikey-list-contains',
			list.out.includes(tokenId),
			`count=${asArray(list.json).length}`,
		);
		...
		if (rok)
			teardown.splice(
				teardown.findIndex(t => t.args[2] === tokenId),
				1,
			);
	} else {
		for (const n of ['apikey-list-contains', 'apikey-get', 'apikey-revoke'])
			skip('S7', n, 'create failed');
	}
	...
	check(
		'S7',
		'access-relationships-list',
		rel.code === 0 && !isErr(rel),
		evidence(rel),
	);
	const grants = cli(['access', 'tool-grants-list']);
	check(
		'S7',
		'access-tool-grants-list',
		grants.code === 0 && !isErr(grants),
		evidence(grants),
	);
	...
	check(
		'S7',
		'isolation-workspace-scoped-list',
		myCount > 0 && myCount < totalAgents,
		`my agents=${myCount} total DB agents=${totalAgents}`,
	);

	skip(
		'S7',
		'cross-user-invite-accept',
		'needs 2nd identity token (second authenticated user)',
	);
}
...
function s8Providers() {
	...
	check(
		'S8',
		'provider-specs-list',
		specs.code === 0 && specArr.length > 0,
		`count=${specArr.length}`,
	);
	...
	check(
		...
		modelInst.code === 0 && !isErr(modelInst),
		`count=${miArr.length}`,
	);
	...
	check(
		'S8',
		'model-specs-list',
		modelSpecs.code === 0 && !isErr(modelSpecs),
		`count=${asArray(modelSpecs.json).length}`,
	);

	// Provider config create -> get -> delete (dummy key)
	const specId =
		specArr.find(s => (s.provider_key || s.key) === 'openrouter')?.id ??
		specArr[0]?.id;
	if (specId) {
		const create = cli([
			'providers',
		...
		const id = isOk(create) ? create.json.id : undefined;
		if (id)
			trackDelete('providers.config-delete', [
				'providers',
				'config-delete',
				id,
			]);
		...
		for (const n of [
			...
			skip('S8', n, 'no provider spec id available');
	}
	...
	if (miArr.length > 0) {
		...
		const test = cli(
			[
				...
				'--data',
				JSON.stringify({
					provider_config_id: mi.provider_config_id,
					model_spec_id: mi.model_spec_id,
					test_message: 'ping',
				}),
			],
			{timeout: 60000},
		...
		check(
			...
			'model-instance-test',
			structured,
			`endpoint returned structured result: ${evidence(test)}`,
		);
	} else {
...
function s9Skills() {
	...
	if (isOk(bad)) {
		if (bad.json.id)
			trackDelete('skills.delete', ['skills', 'delete', bad.json.id]);
		check(
			'S9',
			'neg-skill-missing-name-validation',
			false,
			`empty skill accepted: id=${bad.json.id}`,
		);
		finding(
			'Empty skill create accepted without validation (S9)',
			`POST /v1/skills with {} created skill ${bad.json.id}`,
		);
	} else {
		const st = errStatus(bad);
		check(
...
function s1Connections() {
	...
	const create = cli(
		['connections', 'create', '--data', JSON.stringify(body)],
		{timeout: 60000},
	);
	if (isOk(create) && create.json.id) {
		...
		check(
			'S1',
			'connection-discover-tools',
			disc.code === 0 && !isErr(disc),
			evidence(disc),
		);
		...
		skip(
			'S1',
			'connection-create-from-spec-url',
			`endpoint did not accept public spec URL: ${evidence(create)}`,
		);
...
function s10Bundles() {
	const bad = cli(
		[
			...
			'analyze',
			'--data',
			JSON.stringify({source_url: 'https://example.com/not-a-real-bundle.zip'}),
		],
		{timeout: 60000},
	);
	if (isErr(bad)) {
		...
		check(
			'S10',
			'bundle-analyze-invalid-validation',
			st >= 400 && st < 500,
			`expected clear 4xx (not 500), got status=${st}`,
		);
		if (st >= 500)
			finding('Bundle analyze returns 5xx on bad input (S10)', evidence(bad));
		...
		const flagsProblem = /error|invalid|fail|not.?found|unreachable/i.test(
			blob,
		);
		check(
			'S10',
			'bundle-analyze-invalid-validation',
			flagsProblem,
			`structured report flagged problem: ${blob.slice(0, 200)}`,
		);
	} else {
		check(
			'S10',
			'bundle-analyze-invalid-validation',
			false,
			`no structured response: code=${bad.code} ${(bad.err || bad.out).slice(
				0,
				200,
			)}`,
		);
...
function s12Wallet() {
	...
	if (errStatus(get) === 404)
		finding(
			'No wallet provisioned for real-run agent (S12)',
			`wallet get ${AGENT_REAL} -> 404 (informational, not a failure)`,
		);
}
...
function s13Audit() {
	...
	check(
		'S13',
		'audit-logs-durable',
		list.code === 0 && !isErr(list) && arr.length > 0,
		`events=${arr.length}`,
	);
}
...
function s16Cli() {
	...
	check(
		'S16',
		'cli-bad-token-auth-error',
		bad.code !== 0 && isErr(bad) && st === 401,
		`exit=${bad.code} ${evidence(bad)}`,
	);
}
...
function s19A2a() {
	...
	const looksLikeCard =
		isOk(card) && /name|url|capabilities|version|skills|protocol/i.test(blob);
	check(
		'S19',
		'a2a-agent-card',
		looksLikeCard,
		isOk(card) ? blob.slice(0, 200) : evidence(card),
	);
	...
	const bogus = cli([
		...
		'getAgentWellKnownCardV1AgentsAgentIdWellKnownAgentCardJsonGet',
		'--path',
		JSON.stringify({agent_id: '00000000-0000-0000-0000-000000000000'}),
	]);
	const bst = errStatus(bogus);
	check(
		'S19',
		'a2a-bogus-agent-404',
		isErr(bogus) && (bst === 404 || bst === 403),
		`expected 404, got ${evidence(bogus)}`,
	);
}
...
function outOfScopeNotes() {
	skip(
		'S17',
		'ui-navigation',
		'out-of-CLI-scope: browser UI navigation not drivable via CLI',
	);
	skip(
		'S18',
		'compose-bringup',
		'out-of-CLI-scope: docker compose bringup; stack already up (health checked)',
	);
	for (const s of ['S11', 'S14', 'S15'])
		skip(
			s,
			'scenario',
			'not in assigned CLI coverage set (1-16,19); see wiki for MP',
		);
}
...
function runTeardown() {
	...
	for (const t of teardown) {
		...
		lines.push(
			`  ${ok ? 'deleted' : 'FAILED '} ${t.label} ${t.args[2] ?? ''} ${
				ok ? '' : '-- ' + evidence(r)
			}`,
		);
	}
...
function main() {
	...
	if (tdLines.length === 0)
		console.log(
			'  (nothing to tear down — all created entities already deleted in-scenario)',
		);
	...
	const matrixLines = results.map(
		r =>
			`${r.kind.padEnd(4)}  ${r.scenario.padEnd(5)}  ${r.name.padEnd(38)}  -- ${
				r.evidence
			}`,
	);
