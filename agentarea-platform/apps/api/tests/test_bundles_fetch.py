...
async def test_fetch_returns_text():
    ...
    out = await fetch_bundle_source(
        "http://example.test/bundle.yaml",
        policy=OutboundPolicy(allow_private=True),
        transport=transport,
    )
...
async def test_fetch_raises_on_http_error():
    ...
    with pytest.raises(httpx.HTTPStatusError):
        await fetch_bundle_source(
            "http://example.test/missing.yaml",
            policy=OutboundPolicy(allow_private=True),
            transport=transport,
        )
