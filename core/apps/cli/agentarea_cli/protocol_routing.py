...
class ProtocolRouter:
    ...
    def extract_chunk_content(event_data: dict) -> str:
        ...
        # 3) Common A2A deltas
        # e.g. data: { delta: { text: "..." } }
        delta = event_data.get("delta") or (data_payload.get("delta") if isinstance(data_payload, dict) else None) or {}
        ...
        text = event_data.get("text") or (data_payload.get("text") if isinstance(data_payload, dict) else None)
        if isinstance(text, str) and text:
            return text
        content = event_data.get("content") or (data_payload.get("content") if isinstance(data_payload, dict) else None)
            ...
            async for line in response.aiter_lines():
                ...
                if line_str.startswith('data: '):
                    ...
                    try:
                        ...
                        agent_name, is_streaming_response, current_response = await self._display_event(
                            normalized_event,
                            agent_name,
    ...
    async def _display_event(self, event_data: dict, agent_name: str, is_streaming_response: bool, current_response: str):
        ...
        start_events = {"LLMCallStarted", "LLMRequestStarted", "message.started", "message.start", "MessageStart", "ResponseStarted", "response.started", "llm_call_started", "task_started"}
        chunk_events = {"LLMCallChunk", "LLMResponseChunk", "message.delta", "delta", "chunk", "MessageDelta", "response.delta", "content.delta", "llm_call_chunk"}
        done_events = {"LLMCallCompleted", "LLMResponseCompleted", "message.completed", "message.complete", "MessageCompleted", "response.completed", "response.complete", "message.end", "llm_call_completed"}
        ...
        if clean_event_type in start_events:
            ...
            if not final_text and isinstance(data_payload, dict):
                ...
                result_field = data_payload.get("result") if isinstance(data_payload.get("result"), dict) else None
                final_text = (
