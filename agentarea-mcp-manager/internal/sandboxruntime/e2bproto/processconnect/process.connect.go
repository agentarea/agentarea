...
import (
	...
	context "context"
	errors "errors"
	e2bproto "github.com/agentarea/mcp-manager/internal/sandboxruntime/e2bproto"
	http "net/http"
	strings "strings"
)
...
type ProcessClient interface {
	Start(context.Context, *connect.Request[e2bproto.StartRequest]) (*connect.ServerStreamForClient[e2bproto.StartResponse], error)
}
...
func NewProcessClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) ProcessClient {
	...
	processMethods := e2bproto.File_internal_sandboxruntime_e2bproto_process_proto.Services().ByName("Process").Methods()
	return &processClient{
		start: connect.NewClient[e2bproto.StartRequest, e2bproto.StartResponse](
			httpClient,
...
type processClient struct {
	start *connect.Client[e2bproto.StartRequest, e2bproto.StartResponse]
}

// Start calls process.Process.Start.
func (c *processClient) Start(ctx context.Context, req *connect.Request[e2bproto.StartRequest]) (*connect.ServerStreamForClient[e2bproto.StartResponse], error) {
	return c.start.CallServerStream(ctx, req)
}

// ProcessHandler is an implementation of the process.Process service.
type ProcessHandler interface {
	Start(context.Context, *connect.Request[e2bproto.StartRequest], *connect.ServerStream[e2bproto.StartResponse]) error
}
...
func (UnimplementedProcessHandler) Start(context.Context, *connect.Request[e2bproto.StartRequest], *connect.ServerStream[e2bproto.StartResponse]) error {
