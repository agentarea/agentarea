...
def minio_setup():
    ...
    result = subprocess.run(["mc", "ls", "myminio/documents"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if result.returncode != 0:
        print("Creating MinIO documents bucket...")
        subprocess.run(["mc", "mb", "myminio/documents"], check=True)
        subprocess.run(["mc", "policy", "set", "public", "myminio/documents"], check=True)
