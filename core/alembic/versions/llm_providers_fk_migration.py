...
def upgrade():
    ...
    op.add_column('llm_models', sa.Column('provider_id', postgresql.UUID(as_uuid=True), nullable=True))
    ...
    for row in llm_models:
        ...
        if provider_name not in provider_map:
            ...
            conn.execute(
                sa.text('INSERT INTO llm_providers (id, name, is_builtin, updated_at, created_at) VALUES (:id, :name, :is_builtin, now(), now())'),
    ...
    op.create_foreign_key('fk_llm_models_provider_id', 'llm_models', 'llm_providers', ['provider_id'], ['id'])
...
def downgrade():
    ...
    op.add_column('llm_models', sa.Column('provider', sa.String(), nullable=False, server_default='unknown'))
    ...
    for row in llm_models:
        ...
        provider_name = conn.execute(sa.text('SELECT name FROM llm_providers WHERE id = :id'), {'id': provider_id}).scalar()
        conn.execute(sa.text('UPDATE llm_models SET provider = :provider WHERE id = :model_id'), {'provider': provider_name, 'model_id': row['id']})
