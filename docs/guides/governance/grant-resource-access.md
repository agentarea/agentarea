...
type: guide
description: "Give a workspace member or an agent read, write or manage access to one agent, skill, MCP server or skill collection through the authorization graph."
prerequisites:
...
<Steps titleSize="h3">
  ...
  <Step title="Choose the relation">
...
<AccordionGroup>
  ...
  <Accordion title="`403 Agent:<id> not found in your workspace`">
    The object id is wrong, belongs to another workspace, or you are sending
    it with the wrong `namespace`. The lookup uses the namespace to pick the
  ...
  <Accordion title="`422 Unsupported relation for a resource grant`">
    You sent something outside the accepted set. Use `reader` , `writer` or
    `manager` .
  </Accordion>
  <Accordion title="`422 Group (subject_set) grants are managed via project/role, not the explorer`">
    This endpoint writes direct grants only. Bundled or group-scoped access goes
    through `role` and `role_assignment` objects — see
    [model a custom relation](/guides/governance/model-a-custom-relation) .
  </Accordion>
  <Accordion title="`503 Graph authorization is disabled` or `Graph authorization write failed`">
    The first means `AGENTAREA_AUTHZ_BACKEND` is `disabled` . The second means
    OpenFGA was reachable but rejected or dropped the write; the API
    deliberately does not fall back to allowing the operation, so check the
    OpenFGA container before retrying.
  </Accordion>
  <Accordion title="The grant exists and the user still cannot read">
    Almost always the independent-bits rule. Check all three separately — `read`
    , `edit` and `manage` each need their own tuple, and a `manager` grant
    confers neither of the others.
  </Accordion>
  <Accordion title="The grant exists and nothing changed in the product">
...
<Columns cols={2}>
  ...
  <Card title="Model a custom relation" icon="scale-balanced" href="/guides/governance/model-a-custom-relation">
