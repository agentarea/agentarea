...
class ContextService(Protocol):
    ...
    async def list_events(self, task_id: str, limit: Optional[int] = None) -> List[ContextEvent]: ...
...
def create_context_event_listener(context_service: ContextService, task_id: str) -> Callable[[Any], Any | Awaitable[Any]]:
    ...
    async def _listener(evt: Any) -> None:
        try:
            ...
            await context_service.append_event(task_id, ContextEvent(type=getattr(evt, "type", "event"), payload=getattr(evt, "payload", {})))
        except Exception:
...
def events_to_messages(events: List[ContextEvent]) -> List[Dict[str, Any]]:
    ...
    for ev in events:
        ...
        elif ev.type in {"llm_chunk", "iteration_started", "iteration_finished", "agent_completed", "llm_request", "tool_execution_started", "completion_signaled"}:
