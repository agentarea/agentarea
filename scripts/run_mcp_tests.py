...
async def run_custom_mcp_test(
    ...
    try:
        ...
        if docker_image:
            ...
            response = await client.post(f"{api_base}/v1/mcp-servers/", json=server_data)
            if response.status_code not in [200, 201]:
                raise Exception(f"Failed to create MCP server: {response.status_code} - {response.text}")
        ...
        response = await client.post(f"{api_base}/v1/mcp-server-instances/", json=instance_data)
        if response.status_code not in [200, 201]:
        ...
        response = await client.post(f"{api_base}/v1/agents/{agent_id}/tasks/", json=task_data)
        if response.status_code not in [200, 201]:
        ...
        # Check MCP instance
        response = await client.get(f"{api_base}/v1/mcp-server-instances/{mcp_instance_id}")
        if response.status_code != 200:
        ...
        try:
            ...
            response = await client.get(f"{api_base}/v1/mcp-server-instances/{mcp_instance_id}/tools")
            if response.status_code == 200:
                tools = response.json()
...
def parse_env_vars(env_args: List[str]) -> Dict[str, str]:
    ...
    for env_arg in env_args:
        if '=' not in env_arg:
            raise ValueError(f"Invalid environment variable format: {env_arg}. Use KEY=VALUE")
...
def main():
    ...
    # Custom test options
    parser.add_argument("--image", help="Docker image name for MCP server (e.g., 'myorg/weather-mcp:latest')")
    parser.add_argument("--url", help="MCP server endpoint URL (for already running servers)")
    ...
    # Preset options
    parser.add_argument("--preset", choices=["weather", "filesystem"], 
                       help="Run a preset test")
    
    # General options
    parser.add_argument("--api-base", default="http://localhost:8000",
                       help="AgentArea API base URL")
    ...
    if args.preset:
        ...
        asyncio.run(run_custom_mcp_test(
        ...
        print("❌ Either use --preset, provide --image with optional --env, or provide --url")
        parser.print_help()
        sys.exit(1)
