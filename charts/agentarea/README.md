...
| operator.enabled | bool | `true` | Deploy agentarea-operator for catalog reconcile + LLM provider configs |
| operator.image.repository | string | `"agentarea/agentarea-operator"` |  |
| operator.image.tag | string | `""` | Defaults to chart appVersion |
| registries | list | `[]` | RegistrySync CRs rendered into cluster; see values.yaml for schema |
