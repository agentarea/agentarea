...
def upgrade() -> None:
    ...
    op.add_column('mcp_server_instances', sa.Column('json_spec', sa.JSON(), nullable=False, server_default='{}'))
    
    # Drop foreign key constraint first
    op.drop_constraint('mcp_server_instances_server_id_fkey', 'mcp_server_instances', type_='foreignkey')
    
    # Rename server_id to server_spec_id and change type to String, make nullable
    op.alter_column('mcp_server_instances', 'server_id',
...
def downgrade() -> None:
    ...
    op.add_column('mcp_server_instances', sa.Column('config', sa.JSON(), nullable=False, server_default='{}'))
    ...
    op.alter_column('mcp_server_instances', 'server_spec_id',
    ...
    op.create_foreign_key('mcp_server_instances_server_id_fkey', 'mcp_server_instances', 'mcp_servers', ['server_id'], ['id'])
