"""Integration tests for SkillRepository.get_by_slug (workspace-scoped)."""
...
async def slug_test_engine():
    """In-memory SQLite engine with ONLY the skills table.

    We avoid BaseModel.metadata.create_all because sibling models rely on
    PostgreSQL-only types/tables that SQLite cannot render.
    """
    ...
    async with engine.begin() as conn:
        await conn.run_sync(
            lambda sync_conn: Skill.__table__.create(sync_conn, checkfirst=True)
        )
...
class TestSkillRepositoryGetBySlug:
    @pytest.mark.asyncio
    async def test_get_by_slug_returns_skill(self, slug_db_session: AsyncSession):
        ctx = _ctx()
        repo = SkillRepository(slug_db_session, ctx)
        ...
        retrieved = await repo.get_by_slug("my-skill")
        assert retrieved is not None
        assert retrieved.id == created.id
        assert retrieved.slug == "my-skill"
    ...
    async def test_get_by_slug_other_workspace_returns_none(
        ...
        repo_a = SkillRepository(slug_db_session, ws_a)
        repo_b = SkillRepository(slug_db_session, ws_b)

        await repo_a.create(
            name="Workspace A Skill",
            slug="shared",
            description="Workspace A only",
            source_type="content",
        )

        # Same slug is reusable across workspaces and invisible from another one.
    ...
    async def test_get_by_slug_unknown_returns_none(self, slug_db_session: AsyncSession):
        repo = SkillRepository(slug_db_session, _ctx())
