...
def test_request_budget_only():
    assert resolve_effective_budget(10.0, None) == Decimal("10")
...
def test_policy_tighter_wins():
    ...
    assert resolve_effective_budget(10.0, policy) == Decimal("5.00")


def test_request_tighter_wins():
    policy = {"budget": {"run_budget_usd": "20.00"}}
    assert resolve_effective_budget(8.0, policy) == Decimal("8")
...
def test_policy_without_budget_section_falls_back_to_request():
    assert resolve_effective_budget(12.0, {"tools": {"allowed": ["x"]}}) == Decimal("12")
